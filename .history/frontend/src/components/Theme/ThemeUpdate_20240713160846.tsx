...
export const ThemeUpdate: React.FC<IThemeUpdate> = ({
  ...
  const { updateTheme } = useThemeContext(); // Assurez-vous que `useThemeContext` retourne un objet avec `updateTheme`
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const values: IThemeFormUpdate = {
      images: dataImage,
      title: formData.get("legend") as string,
      groupId: groupId,
      bio: formData.get("dataPhoto") as string,
      themeId: data?.themeId || "", // Assurez-vous d'avoir un `themeId`
    };
