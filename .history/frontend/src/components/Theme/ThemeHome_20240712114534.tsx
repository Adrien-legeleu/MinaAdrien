...
interface ThemeHomeProps {
  redirectToThemePage: () => void; // Prop pour la fonction de redirection
}

export const ThemeHome: React.FC<ThemeHomeProps> = ({
  redirectToThemePage,
}) => {
      ...
      <div className="grid-cols-3 gap-8 grid pt-32 px-8">
        {themes.map((theme) => {
          return (
            <div
              key={theme._id}
              className=" relative bg-white/60 flex items-center justify-center flex-col overflow-hidden pb-7 rounded-3xl gap-8 border-[1px] border-black/10 shadow-2xl shadow-black/00001 "
            >
              <div
              ...
              />
              <div
                onClick={() => redirectToThemePage()} // Utilisation de la fonction de redirection
