      ...
      {theme?.images.map((img: any) => {
        if (img._id === imgId) {
          return (
            <div
              key={img._id}
              className={`z-40  fixed top-0 left-0 py-8 dark:bg-black bg-white  dark:bg-dot-white/[0.2] bg-dot-black/[0.5]   w-screen h-screen flex items-center justify-center ${
                isThemeImgModalOpen
                  ? "visible opacity-100"
                  : "opacity-0 invisible"
            ...
            >
              <div
                ...
                className="absolute pointer-events-none w-screen h-screen inset-0 flex items-center justify-center dark:bg-black bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]"
              ></div>
