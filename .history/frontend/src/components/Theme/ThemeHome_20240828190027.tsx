...
export const ThemeHome = () => {
  ...
  const [extremesDates, setExtremesDates] = useState(["", ""]);
  ...
  const findExtremesDatesToTheme = (themeImages: IImageTheme) => {
    let minDate = Date.now().toLocaleString();
    let maxDate = Date.now().toLocaleString();
    themeImages.map((image) => {
      if (parseDate(image.photoDate) < minDate) {
        minDate = image.photoDate;
      } else if (parseDate(image.photoDate) > maxDate) {
        maxDate = image.photoDate;
      }
    });

    setExtremesDates([minDate, maxDate]);
  };
              ...
              >
                ...
                <div className="px-8 space-y-3 ">
                  <p className="text-center text-2xl font-semibold tracking-wider">
                    {theme.images
                      ? findExtremesDatesToTheme(theme.images)[0]
                      : "Du 23 juillet 2017"}
                  </p>
                  <p className="text-center text-2xl font-semibold tracking-wider">
                    Au 16 mars 2023
