      ...
      {theme?.images.map((img: any) => {
        if (img._id === imgId) {
          return (
            <div
              key={img._id}
              className={`z-40 bg-gray-400 fixed top-0 left-0 py-20  w-screen h-screen flex items-center justify-center ${
                isThemeImgModalOpen
                  ? "visible opacity-100"
                  : "opacity-0 invisible"
            ...
            >
              <img
                src={img.url}
                alt="img theme"
                className="h-full   object-contain rounded-3xl my-20"
              />
