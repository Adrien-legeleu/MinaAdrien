...
import { IThemeForm } from "@/context/ThemeContext";
...
export function ThemeModal() {
  ...
  const handleImageUpload = (imgUrlKey: string, fileList: UploadFile[]) => {
    const uploadedImages: IImage[] = fileList.map((file) => file.url || "");
    setNewImages(uploadedImages);
    changeImageValue(uploadedImages);
  };

  const changeImageValue = (values: IImage[]) => {
    setDataImages(values);
    console.log(values);
  };

  const submitTheme = (e: any) => {
    e.preventDefault();
        ...
        <ModalBody>
          <form onSubmit={submitTheme}>
            ...
            <ModalFooter className="gap-4 ">
              <p className="px-2 py-1 bg-gray-200 text-black dark:bg-black dark:border-black dark:text-white border border-gray-300 rounded-md text-sm w-28">
                Cancel
              </p>
              <button
                type="submit"
