...
export const ParamsGroup = () => {
  ...
  const getMembers = async (group: IGroupComplete | undefined) => {
    ...
    const membersSet = new Set<IUser>(); // Utilisation d'un Set pour éviter les doublons
    ...
    for (const member of group.members) {
      try {
        ...
        membersSet.add(response.data); // Ajouter chaque membre au Set
      } catch (error) {
        console.error("error fetching members");
      }
    }

    // Convertir le Set en tableau et mettre à jour l'état
    setMembers((prevMembers) => [...prevMembers, ...Array.from(membersSet)]);
  };
