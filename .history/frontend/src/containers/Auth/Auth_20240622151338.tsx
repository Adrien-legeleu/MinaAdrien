...
import { IFormValues, useUserContext } from "@/context/UserContexts";
...
export const Auth = () => {
  const { isAuthenticated, onLogin, onRegister } = useUserContext();
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isLogin) {
      await onLogin(values as IFormValues);
    } else {
      await onRegister(values as IFormValues);
    }
  };
