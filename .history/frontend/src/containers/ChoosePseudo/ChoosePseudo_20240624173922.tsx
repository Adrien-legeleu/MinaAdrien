...
import { IPseudoFormValues } from "@/context/GroupContexts";
import { useUserContext } from "@/context/UserContexts";
import { cn } from "@/utils/cn";

export const ChoosePseudo = () => {
  const { choosePseudo, groupId } = useGroupCont;
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const values = {
      groupId: groupId,
      userId: userId,
      pseudoUser: data.get("pseudoUser"),
    };
    console.log(values);
    await choosePseudo(values as IPseudoFormValues);
  };
