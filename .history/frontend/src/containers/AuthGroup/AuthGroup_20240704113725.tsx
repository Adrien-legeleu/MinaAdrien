    ...
    <BackgroundGradientAnimation>
      <div className="absolute inset-0 z-50 flex gap-16 flex-col pt-10 items-center justify-center">
        ...
        {!isGroup ? (
          ...
          <div className="w-2/3 mx-auto relative rounded-t-3xl">
            <div className="h-10 w-full fixed top-0 left-0 backdrop-blur-sm bg-black z-10"></div>
            <div
              className="overflow-y-scroll pt-14"
              style={{ height: "calc(100vh - 40px)", scrollbarWidth: "none" }}
            >
              <div className="grid grid-cols-4 gap-12">
                {user?.groups.map((group: any) => (
                  <div
                    key={group.groupCode}
                    className="bg-white/90 py-5 px-2 flex flex-col items-center justify-between rounded-3xl gap-6"
                  >
                    {group.urlProfil ? (
