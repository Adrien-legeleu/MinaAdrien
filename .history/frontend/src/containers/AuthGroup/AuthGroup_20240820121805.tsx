...
export const AuthGroup = () => {
  ...
  const [groupId, setGroupId] = useState<string | null>(null); // Assurez-vous que groupId est réactif
  const { setUser, user } = useCreateJoinContext();
  const { onDeleteGroup, allGroups, getGroup } = useGroupContext(); // Assurez-vous que getGroup est accessible
  const router = useRouter(); // Utiliser router pour redirection si nécessaire
  ...
  useEffect(() => {
    if (groupId) {
      // Déclenchez le fetch du groupe en fonction du groupId sélectionné
      getGroup();
    }
  }, [groupId]); // Ce useEffect dépend maintenant de groupId
  ...
  const handleGroupClick = (id: string) => {
    ...
    setGroupId(id); // Mettre à jour l'état local
  };
          ...
          >
            <div className="grid grid-cols-4 gap-12 ">
              {allGroups.map((group: any) => (
                <div className="bg-white  space-y-3 border-black/80 border-[1px] relative py-7 px-2  rounded-3xl  cursor-pointer hover:bg-white/90 hover:scale-105 duration-300 ease-in-out">
                  <div
