            ...
            <div className="grid grid-cols-4 max-lg:grid-cols-3 max-md:grid-cols-2 gap-12 max-md:gap-9 max-[340px]:gap-4">
              {allGroups ? (
                allGroups.map((group: any) => (
                  <div
                    key={group._id}
                    className={`space-y-3 bg-[url('${group.urlProfil}')] border-black/60 border-[1px] relative py-7 px-5 rounded-3xl cursor-pointer hover:bg-white/90 hover:scale-105 duration-300 ease-in-out`}
                  >
                    <div
