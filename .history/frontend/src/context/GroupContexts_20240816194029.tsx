...
export const GroupContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getAllGroup = async () => {
    try {
      ...
      const uniqueGroups = new Set<IGroup>(); // Spécifiez que le Set contient des IGroup

      response.data.forEach((group: IGroup) => {
        // Assurez-vous que group est de type IGroup
        group.members.forEach((member: any) => {
          if (member.userId === userId) {
            uniqueGroups.add(group); // Ajoutez le groupe au Set
          }
        });
      });

      setAllGroups(Array.from(uniqueGroups)); // Convertissez le Set en tableau et mettez à jour l'état
    } catch (error: any) {
