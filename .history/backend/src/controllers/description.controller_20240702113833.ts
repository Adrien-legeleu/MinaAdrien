...
export class DescriptionController {
  async findAll(req: any, res: Response): Promise<void> {
    try {
      ...
      const letter = await DescriptionModel.find(groupId);
  ...
  async findOneById(req: any, res: Response): Promise<void> {
    try {
      ...
      const letter = await DescriptionModel.findOne({ groupId, _id: letterId });
      if (!letter) {
        res.status(404).send({
  ...
  async create(req: any, res: Response): Promise<void> {
    try {
      ...
      const letter = await DescriptionModel.create({
        groupId,
        title,
        text,
        url_images,
  ...
  async updateOneById(req: any, res: Response): Promise<void> {
    try {
      ...
      const letter = await DescriptionModel.findOneAndUpdate(
        {
          groupId,
          _id: letterId,
        },
  ...
  async delete(req: any, res: Response): Promise<void> {
    try {
      const { groupId } = req.body;
      const { letterId } = req.params;
      const letter = await DescriptionModel.findOneAndDelete({
        groupId,
        _id: letterId,
      });
      if (!letter) {
