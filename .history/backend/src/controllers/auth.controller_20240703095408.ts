...
export class AuthController {
  ...
  async login(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (isMember) {
        res.status(200).send({ group, authToken, redirect: "HomePage" });
      } else {
        res
          .status(200)
          .send({ group, authToken, redirect: "choosePseudoPage" });
      }
    } catch (err: any) {
      console.log(err);
      res.status(500).send({
  ...
  async register(req: Request, res: Response): Promise<void> {
    try {
      ...
      user?.groups+= {group.password , group._id , group.urlProfil}
